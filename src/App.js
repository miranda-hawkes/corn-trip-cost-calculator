...
function App({tripPlanner, calculate}) {
  ...
  function Pagination() {
    ...
    if(numPages > 0) {
    return (<div>
      <button onClick={previousPage} disabled={currentPage === 0}>Previous</button>Page {currentPage} of {numPages}<button disabled={currentPage >= numPages-1}onClick={nextPage}>Next</button>
      </div>);
    } else {
      return (<div></div>)
    }
